...
if( $nbPage != 0 ) 
{
  ...
  { 
    $pagerContainer .= sprintf( '<a href="' . $link . '"> &#171; page précédente</a>', $page - 1 ); 
  }
  $pagerContainer .= ' <span> page <strong>' . $page . '</strong> sur ' . $nbPage . ' </span>'; 
  if( $page == $nbPage ) 
  { 
    $pagerContainer .= ''; 
  }
  ...
  { 
    $pagerContainer .= sprintf( '<a href="' . $link . '">page suivante &#187; </a>', $page + 1 ); 
  }           
}                   
$pagerContainer .= '</div>';
...
foreach ($contenu2 as $v1){
    echo "<div class='article'>
          ...
          <p class='date'>Posté le ".$v1['ladate']."</p>
          </div>";
}
echo "</main>".$pagerContainer;
?>
